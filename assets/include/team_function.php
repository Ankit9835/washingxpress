...
<?php			
	if(isset($_POST["addTeam"])) {
		...
		if(empty($_POST["designation"])){
			$designationErr = "Description Name is required.";
		} else {
...
if (move_uploaded_file($file_tmp,"teamimage/".$name)) {
	...
	// insert query
	$query = "INSERT INTO team(title, designation, document)VALUES('$title','$designation','$name')";
	
		
			if(!empty($title) && !empty($designation) && !empty($name)) {
...
if (move_uploaded_file($file_tmp,"teamimage/".$name)) {
		...
		if ($_FILES['photo']['name']) /* If there Is  file Selected*/ {
			$uQuery="UPDATE team SET title = '$title', designation = '$designation', document = '$name' WHERE id = '$id'";
    } else /* If file is  Selected*/ {
       $uQuery="UPDATE team SET title = '$title', designation = '$designation' WHERE id = '$id'";
