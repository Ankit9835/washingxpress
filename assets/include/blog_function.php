...
if (move_uploaded_file($file_tmp,"blogimage/".$name)) {
	...
	// insert query
	$query = "INSERT INTO blog(title, description, document)VALUES('$title','$description','$name')";
...
if (move_uploaded_file($file_tmp,"blogimage/".$name)) {
		...
		if ($_FILES['photo']['name']) /* If there Is  file Selected*/ {
			$uQuery="UPDATE blog SET title = '$title', description = '$description', document = '$name' WHERE id = '$id'";
    } else /* If file is  Selected*/ {
       $uQuery="UPDATE blog SET title = '$title', description = '$description' WHERE id = '$id'";
